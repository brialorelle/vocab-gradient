...
test_corpus <- read_csv(here::here("data/item_metadata/test_items.csv")) %>%
  # `honey` and `scrabble` were excluded from the final stimulus set:
  # `scrabble` is a trademarked brand name (IP risk for sharing stimuli);
  # `honey` was dropped because the THINGS+ image was an ambiguous closeup
  # with poor child-name agreement during piloting. See Method §Materials.
  filter(!Word1 %in% c('honey','scrabble'))
```

# New merged data post preprcoessing
```{r}
multiafc_combined <- read_csv(here::here("data/raw/multiAFC_4AFC_clean_combined_data_20251203.csv")) %>%
...
```{r}
clip_cor_by_model <- read_csv(here::here("data/raw/exp1_all_trials2023-04-11.csv")) %>%
...
```{r}
test_clip_cor <- read_csv(here::here("data/raw/exp1_all_trials2023-04-11.csv")) %>%
  select(Word1, Word2, trial_type, cor, AoA_Est_Word2, AoA_Est_Word1) %>%
  rename(wordPairing = trial_type) %>%
  rename(clip_cor = cor) %>%
  filter(Word1 %in% unique(test_corpus$Word1)) %>%
  # `honey` and `scrabble` excluded for symmetry across blocks (see line 36 for rationale).
  filter(!Word1 %in% c('honey','scrabble'))
```
...
adults <- read_csv(here::here("data/raw/multi-afc-april28.csv"))  %>%
  ...
  mutate(cohort = paste0('adults','_', schoolId)) %>%
  filter(task == 'test_response') %>%
  select(pid, cohort, targetWord, answerWord, correct, rt, numAFC, age_group) 
...
manual_pids <- garden %>% 
  select(pid) %>%
  full_join(adults %>% select(pid)) %>%
  full_join(bing %>% select(pid)) %>%
  distinct()
```
...
```{R}
missing = trial_data %>% filter(is.na(age_group))

length(unique(missing$pid))
```
...
df.multiAFC.trials <- trial_data %>% 
  ...
  # Items dropped from the analyzed 108-target set:
  #   honey, scrabble — see comment at line 36 (image ambiguity / trademark)
  #   ant, ball, bear — used as practice trials (not part of the test bank)
...
# Look at participants who are at chance
```{r}
...
In the initial submission, `r length(at_chance$pid)` participants who were older than 6 years of age and at chance on this assessment and completed `r mean(at_chance$num_trials)`

In the revised submission, we included all data.

```{r}
df.4AFC.trials.clean <- df.4AFC.trials %>%
  # filter(!pid %in% at_chance$pid) %>%
  filter(age_group>0) # 1 weird participant with no metadata
```
...
We have `r sum(age_group_pid$num_pids[age_group_pid$age_group < 25])` children aged 3–14 years, and `r sum(age_group_pid$num_pids[age_group_pid$age_group == 25])` adults.
...
```{R}
df.4AFC.distractor.summary.byAge.clean
```

```{r}
save(df.4AFC.distractor.summary.byAge.clean, file=here::here('data/preprocessed/summary_by_distractor.Rdata'))

save(df.4AFC.trials.clean, file=here::here('data/raw/all_trial_data.Rdata'))
```

# Save out raw data that we can share
```{r}
df.4AFC.trials.clean.noschools <- df.4AFC.trials.clean %>%
  filter(cohort != 'schools')

save(df.4AFC.trials.clean.noschools, file=here::here('data/preprocessed/all_trial_data_noschools.Rdata'))
```
