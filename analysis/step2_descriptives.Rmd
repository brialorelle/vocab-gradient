...
```{r}
n_by_age_group <- df.4AFC.trials %>%
  group_by(age_group) %>%
  summarize(num_participants = length(unique(pid)))

trials_by_participant <- df.4AFC.trials %>%
  group_by(pid) %>%
...
```{r}
df.4AFC.distractor.summary.byAge.errors_only <- df.4AFC.distractor.summary.byAge %>%
  ungroup() %>%
...
ggplot(data = df.4AFC.distractor.summary.byAge.perc.byaoa, aes(x=age_group, y=mean, col=wordPairing, fill=wordPairing)) +
  ...
  geom_smooth(aes(group=wordPairing), span=10, alpha=.2) +
  facet_wrap(~AoA_Bin_Name) +
  theme(aspect.ratio=.75) +
  # scale_color_solarized(name='Item') +
  xlab('') +
  ylab('Proportion chosen') +
  theme_few() +
  ylim(0,1) +
...
```{r}
ggplot(data = dist_by_pid_by_age_by_aoa, aes(x=age_group, y=mean, col=wordPairing)) +
  ...
  geom_smooth(data = dist_by_pid_by_age_by_aoa, aes(group=wordPairing), span=20) +
...
```{r}
dist_by_4afc <- df.4AFC.distractor.summary.byPid %>%
  # filter(numAFC==4) %>%
...
dist_by_4afc_trial_count <- df.4AFC.distractor.summary.byPid %>%
  group_by(targetWord, age_group) %>%
  summarize(num_trials = length(unique(pid)), check = n())
...
```{r}
ggplot(data=perc_cor_by_age, aes(x=as.numeric(age_group), y=cor, color=modality, size=num_responses)) +
