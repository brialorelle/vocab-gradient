...
wordcount         : "1486 excluding Method/Results"
...
```{r procedure-figure, echo=FALSE, fig.cap="Example words, their corresponding images, and the distractor categories that varied in similarity to the target concept. ", out.width="100%", fig.align='center'}
...
At an item level, the words that showed the greatest change across age included relatively few animals (e.g., "swordfish"); instead, items with the greatest change tended to included a large variety of inanimate objects ("prism","antenna", "thermos") but also parts of larger buildings ("scaffolding","gutter") (see Figure \ref{fig:slopes} for the top 30 items that showed steep changes in accuracy across development). However,a few relatively easy words (e.g., "cheese") also showed steep changes across development when they were paired with a difficult distractor (e.g., "butter"), suggesting that some of our developmental trends likely also stem from differences in executive control. Thus, this picture vocabulary matching task still assesses many different cognitive abilities beyond the fidelity of children's visual representations.
...
Thus far, these these data support a view where children progressively acquire visual concepts on a relatively long developmental timeline. 
Next, we thus aimed to understand whether we were also observing changes in the precision of children's existing visaul concepts by anlayzing children's error patterns. 
A longstanding idea is that children have initially coarse representations for some of these difficult words that become refined over the development. 
If children's visual concepts are proceeding from overly broad to a more specific representations, then we would expect younger children to be more likely to choose distractors of all types, whereas we would expect older children to almost exclusively choose related distractors. 
Alternatively, if children quickly acquire high-fidelity representations, then we should only observe changes how accurate children are at identifying the target word, with no changes in the types of distractors that children choose when they choose incorrectly.

We thus examined whether we observed systematic changes in how children made errors across age, shown in Figure \ref{fig:errorbyage}. Consistent with this hypothesis, we found that children increasingly choose related distractors throughout development, with adults being still more likely to choose the related distractors relative to the oldest children (14-year-olds) in our sample. 
...
# could also do numtrials as covariate 
...
ggplot(target_item_visualize %>% left_join(item_meta_data), aes(x=higher_semantic_sim, y=mean_prop, col=wordPairing)) +
  geom_point() +
...
ggplot(target_item_visualize %>% left_join(item_meta_data), aes(x=sim_txt_txt, y=sim_img_img, col=wordPairing)) +
  geom_point() +
  geom_smooth(method='lm') +
  theme_few() +
...
```{r}
ggplot(target_item_visualize %>% left_join(item_meta_data), aes(x=AoA_Est_Word1, y=sim_img_img, col=AoA_Bin_Name)) +
  geom_point() +
  geom_smooth(method='lm') +
  theme_few() +
  facet_wrap(~wordPairing) +
  xlab('AoA of Target Word') +
  ylab('Visual sim')
```

```{r}
ggplot(target_item_visualize %>% left_join(item_meta_data), aes(x=AoA_Est_Word1, y=sim_txt_txt, col=AoA_Bin_Name)) +
  geom_point() +
  geom_smooth(method='lm') +
  theme_few() +
  facet_wrap(~wordPairing) +
...
ggsave('figures/similarity_by_item.pdf', width=2.5)
```
...
for (iter in 1:50){
  ...
  lang_model_fit = lmer(data=sampled, prop ~ scale(sim_txt_txt) + scale(age_group) + scale(total_num_errors) +  (1 | targetWord))
  
  vision_model_fit = lmer(data=sampled, prop ~ scale(sim_img_img) + scale(age_group) +  scale(total_num_errors) +  (1 | targetWord))
    
    multi_model_fit = lmer(data=sampled, prop ~ scale(sim_img_txt) + scale(age_group) + scale(total_num_errors) +  (1 | targetWord))
    
    vision_plus_lang_model_fit = lmer(data=sampled, prop ~ scale(sim_img_img) + scale(sim_txt_txt) + scale(age_group) + scale(total_num_errors) +  (1 | targetWord))
    
    all_model_fit = lmer(data=sampled, prop ~ scale(sim_img_txt) + scale(age_group) + scale(sim_img_img) + scale(sim_txt_txt) + scale(total_num_errors) +  (1 | targetWord))
    
        all_model_fit_without_aoa = lmer(data=sampled, prop ~ scale(sim_img_txt) + scale(age_group) + scale(sim_img_img) + scale(sim_txt_txt) + scale(total_num_errors) +  (1 | targetWord))
...
all_model_all_data = lmer(data=dist_by_4afc_by_item_by_age, prop ~ scale(sim_img_txt) + scale(age_group) + scale(sim_img_img) + scale(sim_txt_txt) + scale(total_num_errors) +  (1 | targetWord))

all_model_fit_without_aoa = lmer(data=dist_by_4afc_by_item_by_age, prop ~ scale(sim_img_txt) + scale(age_group) + scale(sim_img_img) + scale(sim_txt_txt) + scale(total_num_errors) +  (1 | targetWord))

# 
r2beta(all_model_all_data, method = "nsj", partial = TRUE)
 out = partR2(all_model_all_data, partvars = c("scale(sim_txt_txt)", "scale(sim_img_img)", "scale(sim_img_txt)"))
```
...
all_model_r2_long <- all_model_r2 %>%
  rename('All predictors' = all, 'Multimodal similarity' = multi, 'Linguistic similarity' = lang, 'Visual similarity' = vision) %>%
  pivot_longer(cols=everything(), values_to="cor", names_to = "model")  %>%
  mutate(model = fct_relevel(model,  'All predictors','Multimodal similarity','Linguistic similarity','Visual similarity'))
...
knitr::include_graphics("figures/embedding_similarity.png")
```
...
ggsave('figures/model_comparison.pdf', height=3, units='in')
