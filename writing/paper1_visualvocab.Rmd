...
model_sanity = lmer(prop_hard ~ scale(age_group) + scale(total_num_errors) + (1|pid), data = error_by4afc_for_glmer %>% mutate(prop_hard = hard/total_num_errors))
```
...
kable(table_data, 
      format = "latex",
      booktabs = TRUE,
      caption = "Fixed effect coefficients a linear mixed effects model assessing changes in the proportion of related distractors chosen over development. The model included random intercepts for participants. Age and number of trials were standardized prior to analysis.",
      align = c("l", "r", "r", "r", "r")) %>%
