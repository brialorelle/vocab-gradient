...
Most empirical work on the precision of children’s visual concepts has focused on the first few years of life, when children learn words at an astonishing rate [@bloom_how_2000; @braginsky_consistency_2019]. In looking-while-listening tasks [@fernald2008looking], infants as young as 6 months look to events associated with common nouns like “bottle” and “banana” [@bergelson2017nature], and 14- to 18-month-olds extend newly learned words to atypical exemplars of these categories  [@weaver2024becoming]. By around their second birthday, children also generalize nouns to stylized, 3D exemplars [@pereira2009developmental] as they learn that shape is a valuable cue to basic-level category membership. In parallel, empirical work on children’s developing ability to recognize objects [@ayzenberg_development_2024] has focused on these first few years of childhood as an important period of developmental change.
...
trials_by_participant_by_school_type <- trials_by_participant %>%
...
ggplot(trials_by_participant_by_school_type %>% mutate(cohort = school_type), aes(x=age_group, y=mean, col=cohort, fill=cohort)) +
...
main_results = lmer(data = d, perc ~  scale(log_freq_target_word)  + concreteness_target_word + scale(mean_typicality) + n + scale(age_group)*scale(AoA_Est_Word1) + (1|targetWord))
...
apa_table(
  apa_lm$table
  , caption = "Linear mixed-effect model coefficients predicting the proportion correct for each item as a function of it's frequency, concreteness, typicality,and estimated AoA. All predictors were standardized before analysis."
)
```
...
ggsave('figures/fig2a_itemeffects.pdf', width=3.5, height=3, units='in')
```
...
ggsave('figures/fig3-errors.pdf', width=4, height=4, units='in')
```
...
```{r}
all_data_to_model <- dist_by_4afc_by_item_by_age  

all_data_to_model$sem_resid <- resid(
  lm(sim_txt_txt ~ sim_img_img, data = all_data_to_model)
)

resid_model_fit = lmer(data=all_data_to_model, prop ~  scale(sim_img_txt)*scale(age_group) + scale(AoA_Est_Word1) + scale(total_num_errors) +  (1 | targetWord))

summary(resid_model_fit)
```
...
```{r}
exploratory_model_fit  = lmer(data=all_data_to_model, prop ~ scale(AoA_Est_Word1) + scale(AoA_Est_Word2) + scale(sem_resid)*scale(age_group) + scale(phon_sim) + scale(sim_img_txt) + scale(log_freq_target_word) + scale(log_freq_answer_word) + scale(concreteness_target_word) + scale(total_num_errors) +  (1 | targetWord))

summary(exploratory_model_fit)
```
...
maximal_model  = lmer(data=all_data_to_model, prop ~ scale(sem_resid)*scale(age_group) + scale(phon_sim) + scale(AoA_Est_Word1) + scale(sim_img_txt)*scale(age_group)  + scale(total_num_errors) +  (1 | targetWord))

maximal_modl_without_phon = lmer(data=all_data_to_model, prop ~ scale(sem_resid)*scale(age_group) + scale(AoA_Est_Word1) +  scale(sim_img_txt)  + scale(total_num_errors) +  (1 | targetWord))

max_modl_without_multimodal = lmer(data=all_data_to_model, prop ~ scale(sem_resid)*scale(age_group) + scale(phon_sim)   + scale(AoA_Est_Word1) + scale(total_num_errors) +  (1 | targetWord))


max_modl_without_aoa = lmer(data=all_data_to_model, prop ~ scale(sem_resid)*scale(age_group) + scale(phon_sim) + scale(total_num_errors) +  (1 | targetWord))


anova(maximal_model,maximal_modl_without_phon)

anova(maximal_model,max_modl_without_multimodal)

anova(maximal_model,max_modl_without_aoa)
```
...
ggsave('figures/broad_category_accuracy.pdf', width=4, height=4, units='in')
...
```{r supplemental-aoa-plot, include=FALSE}
ggplot(d %>% filter(age_group<25), aes(x=AoA_Est_Word1, y=perc, col=age_group, fill=age_group, size=n)) +
  scale_color_viridis_c() +
  scale_fill_viridis_c() +
  scale_size_area(name="", guide = "none") +
  geom_line(aes(group=targetWord), col='grey', alpha=.1) +
  geom_point(alpha=.4) +
  geom_smooth(aes(group=age_group), method='lm', alpha=.2) +
  theme_few() +
  ylab('Proportion correct') +
  xlab('Estimated AoA') 
  # facet_wrap(~age_group)

ggsave('figures/supplemental-aoa-plot.pdf', width=4, height=4, units='in')
