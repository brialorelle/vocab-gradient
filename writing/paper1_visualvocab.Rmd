...
dist_by_4afc_by_item_by_age <- df.4AFC.distractor.summary.byPid %>%
  ...
  left_join(item_meta_and_model_sim %>% distinct(targetWord, wordPairing, sim_img_img, sim_img_txt, sim_txt_txt, phon_sim, log_freq_target_word, concreteness_target_word)) 
...
resid_model_fit = lmer(data=all_data_to_model, prop ~ scale(sem_resid)*scale(age_group) + scale(sim_img_txt) + scale(AoA_Est_Word1) + scale(total_num_errors) +  (1 | targetWord))

summary(resid_model_fit)
```

```{r}
ggplot(all_data_to_model, aes(x=sem_resid, y=prop, col=sim_img_txt)) +
  geom_point(alpha=.8) +
...
```{r}
exploratory_model_fit  = lmer(data=all_data_to_model, prop ~ scale(sem_resid)*scale(age_group) + scale(phon_sim)*scale(age_group) + scale(sim_img_txt) + scale(log_freq_target_word) + scale(concreteness_target_word) + (AoA_Est_Word1) + scale(total_num_errors) +  (1 | targetWord))
```
